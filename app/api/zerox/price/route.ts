...
export async function POST(request: NextRequest) {
  try {
    const { chainId, sellToken, buyToken, sellAmount } = await request.json()
    ...
    // Build the URL
    const url = new URL(`${apiUrl}/swap/v1/price`)
    ...
    console.error("Error fetching 0x price:", error)
    return NextResponse.json({ error: "Failed to fetch 0x price" }, { status: 500 })
  }
}
